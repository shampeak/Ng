...
class Input {
    ...
    //todo 简单修改,后面更新
    //对 get post server的数据获取
    public static function get($key = null,$xss_clean = false) {
        if($key){
            $val = $_GET[$key];
        }else{
            $val = $_GET;
    ...
    public static function post($key = null,$xss_clean = false) {
        if($key){
            $val = $_POST[$key];
        }else{
            $val = $_POST;
    ...
    public static function server($key = null,$xss_clean = false) {
        if($key){
            $val = $_SERVER[$key];
        }else{
            $val = $_SERVER;
    ...
    public static function session($key = null, $xss_clean = null) {
        if($key){
            $val = $_SESSION[$key];
        }else{
            $val = $_SESSION;
...
//    }
     ...
     * @return type
     * cookies 设置 server('cookies')->set('name',"irones",10);
     */
    public static function cookie($key = null, $xss_clean = false) {
        $cookies = server('cookies')->get($key);
        return $xss_clean ? self::xss_clean($cookies) : $cookies;
    }
