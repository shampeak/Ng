...
$system = [
    ...
    'controller_folder' => dirname(__FILE__) . '/application/controllers',      //存放控制器文件的文件夹路径名称
    'model_folder'      => dirname(__FILE__) . '/application/models',           //存放模型文件的文件夹路径名称,支持数组
    'view_folder'       => dirname(__FILE__) . '/application/views',            //存放视图文件的文件夹路径名称,支持数组
    'library_folder'    => dirname(__FILE__) . '/application/library',          //存放类库文件的文件夹路径名称,存放在该文件夹的类库中的类会自动加载,支持数组
    'helper_folder'     => dirname(__FILE__) . '/application/helper',           //存放函数文件的文件夹路径名称,支持数组
    'table_cache_folder'=> dirname(__FILE__) . '/application/cache',            // table()方法缓存表字段信息的文件夹路径名称
    'hmvc_folder'       => dirname(__FILE__) . '/application/modules',          //存放HMVC模块的文件夹路径名称
     ...
     */
    'error_page_404' => 'application/error/error_404.php',

    /**
     * 系统错误文件的路径,该文件会在发生Fatal错误和Exeption时显示,
     * 文件里面可以使用$msg变量获取出错提示内容
     */
    'error_page_50x' => 'application/error/error_50x.php',
    ...
    'model_file_subfix'         => '.model.php',    //模型文件名称后缀,比如.model.php
    'view_file_subfix'          => '.view.php',     //视图文件名称后缀,比如.view.php
    'library_file_subfix'       => '.class.php',    //类库文件名称后缀,比如.class.php
    'helper_file_subfix'        => '.php',          //函数文件名称后缀,比如.php
     ...
     */
    'models_file_autoload' => array(),


    'controller_method_ucfirst' => TRUE,        //控制器方法名称是否首字母大写,默认true
    'autoload_db'       => FALSE,               //是否自动连接数据库,默认FALSE

    'debug'             => TRUE,                //是否开启调试模式          true：显示错误信息,                    false：所有错误将不显示

    'error_manage'      => FALSE,               //是否接管错误信息显示        true：所有错误信息将由系统格式化输出 false：所有错误信息将原样输出


    //remove
    'default_timezone'  => 'PRC',                // 默认时区,PRC是中国
...
return [
    ...
    'Cookie'=>[
        ...
        'securekey'=> 'ekOt4_Ut0f3XE',      // encrypt key   密钥
        'expire'   => 36000,                //超时时间
    ],
